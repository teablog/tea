...
func (*_tools) PreserveHost(ctx *gin.Context) {
...
func (*_tools) Weather(ctx *gin.Context) {
...
func (*_tools) Ip(ctx *gin.Context) {
	ip, _ := ctx.GetQuery("ip")
	res, err := tools.LocationByIp(ctx, ip)
	if err != nil {
		helper.Fail(ctx, err)
		return
	}
...
func (*_tools) Amap(ctx *gin.Context) {
	...
	if address, err := tools.Location.FindByGeoCode(ctx, latitude, longitude); err != nil {
		helper.Fail(ctx, err)
		return
	} else {
...
func (*_tools) Location(ctx *gin.Context) {
	latitude := ctx.Query("latitude")
	longitude := ctx.Query("longitude")
	if latitude != "" && longitude != "" {
		if address, err := tools.Location.FindByGeoCode(ctx, latitude, longitude); err == nil {
			if address.AdCode != "" {
				if res, err := tools.AdCode.Component(ctx, string(address.AdCode)); err == nil {
					helper.Success(ctx, res)
					return
				}
			}
	...
	ip := ctx.ClientIP()
	if location, err := tools.LocationByIp(ctx, ip); err == tools.LocationError {
		helper.Success(ctx, tools.AdCode.NewDefault())
		return
	} else if err != nil {
		helper.Fail(ctx, err)
	} else {
		helper.Success(ctx, location)
		return
	}
}
